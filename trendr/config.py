...
PASSWORD_SCHEMES = fetch_config("PASSWORD_SCHEMES", default=defaults["PASSWORD_SCHEMES"])
SQLALCHEMY_DATABASE_URI = fetch_config("SQLALCHEMY_DATABASE_URI", default=defaults["SQLALCHEMY_DATABASE_URI"])
if SQLALCHEMY_DATABASE_URI == defaults["SQLALCHEMY_DATABASE_URI"]:
    print("WARNING: Local database being used")

SQLALCHEMY_TRACK_MODIFICATIONS = False

CELERY_BROKER_URL = fetch_config("CELERY_BROKER_URL", default=defaults["CELERY_BROKER_URL"])
if CELERY_BROKER_URL == defaults["CELERY_BROKER_URL"]:
    print("WARNING: Local insecure celery broker being used")

CELERY_RESULT_BACKEND = fetch_config("CELERY_RESULT_BACKEND", default=defaults["CELERY_RESULT_BACKEND"])
